...
<script>
// 获取歌曲详情和 歌曲的歌词接口
import { getSongApi, getLyricApi } from '@/api'
...
export default {
  ...
  data() {
    return {
      ...
      id: this.$route.query.songId, // 上一页传过来的音乐id
      songInfo: {}, // 歌曲信息
  ...
  methods: {
    async getSong() { // 获取歌曲详情, 和歌词方法
      const res = await getSongApi({ids:this.id})
      this.songInfo = res.data.songs[0]
      // 获取-并调用_formatLyr方法, 处理歌词
      const lyrContent = await getLyricApi(this.id)
